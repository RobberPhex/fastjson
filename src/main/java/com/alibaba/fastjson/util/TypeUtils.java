...
public class TypeUtils {
    ...
    public static Annotation[][] getParameterAnnotations(Constructor constructor) {
        ...
        if (mixInClass != null) {
            ...
            if (mixInAnnotations != null) {
                if (parameterTypes.length != mixInAnnotations.length) {
                    assert (level + parameterTypes.length == mixInAnnotations.length);
                    Annotation[][] newMixInAnnotations = new Annotation[parameterTypes.length][];
                    System.arraycopy(mixInAnnotations, level, newMixInAnnotations, 0, parameterTypes.length);
                    mixInAnnotations = newMixInAnnotations;
                }
                return mixInAnnotations;
            }
        }
